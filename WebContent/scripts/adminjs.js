...
/* 侧边菜单 */
jQuery(dropDownSideMenu($));

function dropDownSideMenu() {
  var sideMenuNavUl = $(".side-menu-nav-ul");
  sideMenuNavUl.on("mouseenter", ".menu-title", function (e) {
    e.preventDefault();
    var menuTitle = $(this);
    var menuItem = menuTitle.parent();
    var parent = menuTitle.parent().parent();
    var subMenuItem = parent.children().children(".menu-ul").not(menuItem);
    var menuUl = menuTitle.next();
    if (menuUl.length == 0) return;
    subMenuItem.stop().slideUp("fast");
    menuUl.stop().slideDown("fast");
  });
}

jQuery(menuitemChangeIframe($));

function menuitemChangeIframe() {
  var menuItem = $(".menu-item");
  var iframe = $("iframe");
  console.log($(window).height() + $(".adminMenu").height());
  iframe.height($(window).height() + $(".adminMenu").height());
  menuItem.click(function (e) {
    var href = $(this).attr("data-href");
    if (!href) {
      href = "http://www.baidu.com";
    }
    iframe.attr("src", href).height(
      $(window).height() + parseInt($(".adminMenu").height()));
  });
}
...
function slideContentBottomSliderAsMouseEnter() {
  ...
  var contentBottomSlider = bottomSlider.find(".content-bottom-slider");
  ...
  li.on({
    "mouseenter": function () {
      var firstLiOffsetLeft = getFirstLiOffsetLeft(bottomSlider);
    ...
    "click": function () {
      li.removeClass("op-tab-active");
      $(this).toggleClass("op-tab-active");
      var firstLiOffsetLeft = getFirstLiOffsetLeft(bottomSlider);
      var ooWidth = $(this).outerWidth();
      var otherLiOffsetLeft = $(this).offset().left;
      ...
      li.find("a").css("color", "#c1c5c6");
      $(this).find("a").css("color", "#ffffff")
    }
  });
}

function getFirstLiOffsetLeft(parent) {
  return parent.find("li:first").offset().left;
}


/* 选项卡 */
/* 点击按钮获取属性[chsName、enName、href]，然后在指定地方新建div包含iframe */

/* 点击close-tab删除元素tab li和tab div-frame */
jQuery(colseTab($));
...
function colseTab() {
  ...
  closeTab.click(function (e) {
    ...
    tabLi.fadeOut().remove();
    var tabDivName = $(this).prev().attr("href").substring(1);
    var tabDiv = $("div[id=" + tabDivName + "]");
    tabDiv.fadeOut().remove();
  });
}


function dynamicCreateTabFrame() {

}

/* 随窗口变化初始化侧边菜单 */
function initSideMenu(sideMenu, className) {
  var mask = $(".mask");
  var windowWidth = $(window).width();
  if (windowWidth < 768) {
    if (!sideMenu.hasClass(className)) {
      sideMenu.addClass(className).css("left", "-50rem");
    }
  } else {
    mask.css("display", "none");
    sideMenu.removeClass(className).css("left", "0");
  }
}
...
function reSizeSideMenu() {
  ...
  var xsSideMenuClassName = "side-menu-xs";
  initSideMenu(sideMenu, xsSideMenuClassName);
  $(window).resize(function () {
    initSideMenu(sideMenu, xsSideMenuClassName);
  });
  // 操作侧边菜单
  ...
  for (var [key, value] of fieldMap) {
    ...
    if ("INPUT" == tag) {
      if ("radio" == type) {
        ...
        if (checkedFormEle.next().text() == value) {
          i += 1;
        }
...
function loadFieldMapDataFromTableToFormInModalOnClickBtnSelAndParentSel(modal, table, form, parentsSel, editBtnSel) {
  var fieldMap = new Map();
  var fieldTextMap = new Map();
  ...
  /* jQuery内置委托 */
  /* 编辑 */
  $(parentsSel).on("click", editBtnSel, function (e) {
    // console.log("fdfdf")
    var btn = $(this);
    // 加载字段到表单元素
  ...
  modal.on('hide.bs.modal', function () {
    ...
    clearDataInEleAsFieldTextMap(form, fieldTextMap);
  })
}
  ...
  fieldName: {
    ...
    validators: {
      ...
      stringLength: {
        min: 0,
        max: 0,
        message: "字段名长度必须在 min - max 之间"
      },
      identical: {
        field: otherFieldName,
        message: "字段名必须与其他字段名相同",
      },
      phone: {
        country: CN,
        message: "手机号码格式必须合法",
...
jQuery(viewAdminArticleModal($));

function viewAdminArticleModal() {
  var modal = $("#admin-view-article-modal");
  var form = $("#admin-view-article-form-modal");
  var table = $("#article-list-table");
  // initFormValidErrMsg(form);

  loadFieldMapDataFromTableToFormInModalOnClickBtnSelAndParentSel(modal, table, form, ".article-list-container", ".view-article-link");
}

/* 编辑文章模态窗口 */
jQuery(editAdminArticleModal($));

function editAdminArticleModal() {
  var modal = $("#admin-edit-article-modal");
  var form = $("#admin-edit-article-form-modal");
  var table = $("#article-list-table");
  // initFormValidErrMsg(form);

  loadFieldMapDataFromTableToFormInModalOnClickBtnSelAndParentSel(modal, table, form, ".article-list-container", ".edit-article-link");
}

/* 删除文章模态窗口 */
jQuery(deleteAdminArticleModal($));

function deleteAdminArticleModal() {
  var modal = $("#admin-edit-article-modal");
  var table = $("#article-list-table");

  deleteInModalOnClickBtnSelAndParentSel(modal, table, ".article-list-container", ".del-article-link", "admindeleteArticle");
}
...
/* modal、form、fieldMap、table btn及其父元素 */

/* 编辑用户模态窗口 */
jQuery(editAdminUserModal($));

function editAdminUserModal() {
  var modal = $("#admin-edit-user-modal");
  var form = $("#admin-edit-user-form-modal");
  var table = $("#user-list-table");

  loadFieldMapDataFromTableToFormInModalOnClickBtnSelAndParentSel(modal, table, form, ".user-list-container", ".edit-user-link");
...
/* 删除用户模态窗口 */
jQuery(deleteAdminUsrModel($));

function deleteAdminUsrModel() {
  var modal = $("#admin-delete-user-modal");
  var table = $("#user-list-table");
  deleteInModalOnClickBtnSelAndParentSel(modal, table, ".user-list-container", ".del-user-link", "admindeleteUser");
}
...
/* 编辑用户信息模态窗口 */
jQuery(editAdminUserInfoModal($));

function editAdminUserInfoModal() {
  var modal = $("#admin-edit-userinfo-modal");
  var form = $("#admin-edit-userinfo-form-modal");
  var table = $("#userinfo-list-table");

  loadFieldMapDataFromTableToFormInModalOnClickBtnSelAndParentSel(modal, table, form, ".userinfo-list-container", ".edit-userinfo-link");
}
...
function getAddressJSONByAjaxAndInit() {
  if (!addressData) {
    $.getJSON('scripts/provinceCity2018.json', function (data) {
      addressData = data;
